...
const UserList = ({ users }) => {
  return (
    <div className="w-[550px] bg-white border-slate-50 border-[1px] shadow-md rounded-2xl h-auto  flex flex-col  justify-center items-center">
      ...
      <ul className=" w-full h-80 bg-white  overflow-y-auto">
        {users.map((user) => (
          <Link
            key={user.id}
            to={`/profile/${user.id}`}
